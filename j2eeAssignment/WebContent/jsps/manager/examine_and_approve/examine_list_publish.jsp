...
<%
    ...
    ArrayList<Restaurant_publish_application_info> rests= (ArrayList<Restaurant_publish_application_info>) ServiceFactory.getRestaurant_publish_application_infoManageService().getAllPublishApplications();
...
<div>
    ...
    <!--商家-->
    <form action="/j2eeAssignment/manager/to_publish" method="get">
    <div style="min-height:800px; width: 80%;border: 1px solid green;float:left;box-sizing: border-box">
        <table class="table" id="project">
            <th>审批</th><th>商家ID</th><th>新商品类型</th><th>新商品名称</th>
            <%
                for(int i=0;i<rests.size();i++){
                    Restaurant_publish_application_info r=rests.get(i);
                    //注册新商品类型
                    String com_type="";
                    if(r.getCom_type()==1){
                        com_type="单品";
                    }
                    else if(r.getCom_type()==1){
                        com_type="套餐";
                    }
                    else{
                        com_type="优惠";
                    ...
                    out.println("<tr>\n" +
                            "                <td><button type=\"submit\" value=\""+r.getId()+"\" name=\"publish_id\">审批</button></td>\n" +
