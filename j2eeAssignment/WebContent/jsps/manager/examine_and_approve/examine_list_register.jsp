...
<%
    ...
    ArrayList<Restaurant_register_application_info> rests= (ArrayList<Restaurant_register_application_info>) ServiceFactory.getRestaurant_register_application_infoManageService().getAllRegisterApplications();
...
<div>
    ...
    <!--商家-->
    <form action="/j2eeAssignment/manager/to_register" method="get">
    <div style="min-height:800px; width: 80%;border: 1px solid green;float:left;box-sizing: border-box">
        <table class="table" id="project">
            <th>审批</th><th>注册商家名称</th><th>注册商家类型</th><th>注册商家地址</th>
            <%
                for(int i=0;i<rests.size();i++){
                    Restaurant_register_application_info r=rests.get(i);
                    //注册商家类型
                    String type_name=ServiceFactory.getRestaurant_typesManageService().getTypeNameById(r.getType_id());
                    out.println("<tr>\n" +
                            "                <td><button type=\"submit\" value=\""+r.getId()+"\" name=\"register_id\">审批</button></td>\n" +
                            "                <td>"+r.getName()+"</td>\n" +
                            "                <td>"+type_name+"</td>\n" +
                            "                <td>"+r.getAddress()+"</td>\n" +
                            "            </tr>");
