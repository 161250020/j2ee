...
/**
 * Servlet implementation class statistic_consume
 */
@WebServlet("/member/statistic_consume")
public class statistic_consume extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public statistic_consume() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(int i=0;i<mois.size();i++) {
			if(mois.get(i).getMember_id().equals(m.getId())&&(mois.get(i).getState()==3)) {
		...
		session.setAttribute("consume_start_time", order_start_time);
		session.setAttribute("consume_end_time", order_end_time);
