...
/**
 * Servlet implementation class unsubscribe_get_orders
 */
@WebServlet("/restaurant/unsubscribe_get_orders")
public class unsubscribe_get_orders extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public unsubscribe_get_orders() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(int i=0;i<mois.size();i++) {
			if((mois.get(i).getRestaurant_id().equals(m.getLogin_id()))&&(mois.get(i).getResult()==0)) {
		...
		session.setAttribute("unsubscribe_rest_start_time", order_start_time);
		session.setAttribute("unsubscribe_rest_end_time", order_end_time);
